...
async function getGuideBySlug(slug: string): Promise<Guide> {
  const { data } = await axios.get(
    `${env.NEXT_PUBLIC_STRAPI_REST_API_ENDPOINT}/guides?filters[slug][$eqi]=${slug}&populate=*&publicationState=preview`
  );
...
export { getAllGuides, getGuideBySlug, getGuideById };
