...
function CharacterCalculator(): JSX.Element {
    ...
    const [singleMaterialValues, setSingleMaterialValues] = useState(getSingleMaterialValues());
    ...
    useEffect(() => {
        setCurrentCharacterLevel(capValue(currentCharacterLevel, 1, targetCharacterLevel));
        setSingleMaterialValues(getSingleMaterialValues());
    }, [targetCharacterLevel]);

    useEffect(() => {
        setTargetCharacterLevel(capValue(targetCharacterLevel, 1, getMaxTargetCharacterLevel()));
        setCurrentCharacterLevel(capValue(currentCharacterLevel, 1, targetCharacterLevel));
        setSingleMaterialValues(getSingleMaterialValues());
    }, [characterRarity]);
    ...
    function getSingleMaterialValues(): MaterialAmount[] {
        let accExp = getAccumulatedExp();

        let result = [];
        for (let index = 0; index < materials.length; index++) {
            result.push(new MaterialAmount(materials[index].name, Math.ceil(accExp / materials[index].exp)));
        }
    ...
    function getAccumulatedExp() {
        if (targetCharacterLevel - currentCharacterLevel == 0)
            return 0;

        let result = 0;
        for (let index = currentCharacterLevel - 1; index < targetCharacterLevel - 1; index++) {
                ...
                <div>
                    <label>
                        ...
                        <input
                            ...
                            onChange={(e) => setCurrentCharacterLevel(capValue(Number(e.target.value), 1, targetCharacterLevel))}
                        />
                    </label>
                </div>
                <div>
                    <label>
                        ...
                        <input
                            ...
                            onChange={(e) => setTargetCharacterLevel(capValue(Number(e.target.value), 1, getMaxTargetCharacterLevel()))}
                        />
                    </label>
                </div>
                <div>
                    <label>
                        ...
                        <input
                            ...
                            min="2"
                            max="4"
                            onChange={(e) => setCharacterRarity(capValue(Number(e.target.value), 2, 4))}
                        />
                    </label>
                </div>
                <div>
